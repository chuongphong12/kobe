    ...
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h1 class="page-title">Sản phẩm sữa</h1>
                <ul class="breadcrumb">
                    <li><a href="/">Trang chủ</a></li>
                    <li class="current"><span>Sản phẩm sữa </span></li>
                </ul>
            </div>
        </div>
    </div>
                ...
                <div class="col-xl-4 offset-xl-1 col-lg-5 product-main-details mt-md--50">
                    <div class="product-summary pl-lg--30 pl-md--0">
                        ...
                        <p class="product-short-description mb--40">
                            {{$product->description}}</p>
                        <h4 class="unit-price mb--15">Đơn giá: {{number_format($product->cost,0)}}đ/1000ml</h4>
                        <p class="product--in-stock mb--30">Còn hàng</p>

                        <form action="{{route('cart.add', $product->id_product)}}" method="POST"
                            class="variation-form mb--30">
                            @csrf
                            <div class="product-size-variations d-flex align-items-center mb--30">
                                <p class="variation-label">Dung tích:</p>
                            ...
                            <div
                                class="product-action d-flex flex-sm-row align-items-sm-center align-items-start mb--30">
                            ...
                            <div class="product-price-wrapper mb--40">
                                <span class="money_key" style="font-size: 32px">Tổng tiền:
                            ...
                            <div class="tab-pane fade" id="nav-reviews" role="tabpanel"
                                ...
                                <div class="product-reviews">
                                    <ul class="review__list mb--40">
                                        @if(Auth::check())
                                            <h3>Xin chào {{Auth::user()->username}} bạn hãy để lại đánh giá về {{$product->name}}</h3>
                                        @else
                                            <h3>Vui lòng đăng nhập để đánh giá {{$product->name}}</h3>
                                        @endif
                                    <li class="review__item">
                                        <form action="{{ route('rating') }}" method="POST">
                                            @csrf
                                            <input name="rate" required class="rating" data-size="lg">
                                            <input type="hidden" name="id" required="" value="{{ $product->id_product }}">
                                            <button type="submit" class="btn btn-primary">Đánh giá</button>&nbsp;
                                        </form>
                                        <div>{{$product->name}} đã có {{count($product->ratings)}} đánh giá từ các người dùng khác nhau</div>
                                    </li>
                                    </ul>
                                    
                                    <!-- Comment Area Start -->
                                    <section class="comment">
                                        <div class="fb-comments" data-href="{{route('product',$product->slug)}}" data-width="100%" data-numposts="15"></div>
                                    </section>
                                    <!-- Comment Area End -->
                  ...
                  <div class="product-info">
                    ...
                    <div class="product-info-bottom">
                      ...
                      <div class="product-price-wrapper">
                        ...
                        <span class="money">{{number_format($item->cost,0)}}₫</span>

                      </div>

                      <a href="{{route('cart.add', $item->id_product)}}" class="add-to-cart pr--15">
...
<input type="hidden" name="unit-price" id="unit-price" value="{{$item->cost}}">
